...
func (ps *playtechScraper) GetJobs(ctx context.Context) ([]domain.Job, error) {
	html, err := ps.retriever.Fetch(ctx, ps.url)
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve Playtech html: %w", err)
	}

	jobs, err := ps.parseJobs(html)
	if err != nil {
		return nil, fmt.Errorf("failed to parse Playtech jobs: %w", err)
	}
